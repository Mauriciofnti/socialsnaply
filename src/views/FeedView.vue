      ...
      <div v-else class="posts-list">
        <div v-for="post in postsStore.posts" :key="post.id" class="post-card">
          ...
          <div class="post-actions">
            ...
            <router-link :to="`/profile/${post.author?.id || post.id}`" class="profile-link">Ver Perfil</router-link>
            <!-- ← NOVO: Botões edit/delete se for o próprio post -->
...
.new-post-form textarea { width: 100%; min-height: 80px; padding: 10px; border: 1px solid #ddd; border-radius: 4px; resize: vertical; }
.new-post-form button { background: #28a745; color: white; border: none; padding: 10px; margin-top: 10px; cursor: pointer; }
...
.profile-link { margin-left: auto; text-decoration: none; color: #007bff; }
.loading, .empty-feed { text-align: center; padding: 40px; color: #666; }
</style>